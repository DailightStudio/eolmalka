...
npx eas-cli env:create --environment production --name EXPO_PUBLIC_NEWS_PROXY_URL --value https://jxvjynujqecwebavmkyx.supabase.co/functions/v1/news-sentiment --visibility plaintext --force
...
REM 테스트 광고 강제 (검증용). 켜려면 아래 줄 주석 해제 후 실행. ⚠️ 스토어 출시 전 반드시 다시 제거/0 처리.
REM npx eas-cli env:create --environment production --name EXPO_PUBLIC_USE_TEST_ADS --value 1 --visibility plaintext --force
